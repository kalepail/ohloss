...
///
/// Tests that verify a real game contract (number-guess) works correctly with Ohloss:
/// - Deploy and register the number-guess contract
/// - Start a game session through Ohloss
...
use super::testutils::{
    assert_contract_error, assert_number_guess_error, create_ohloss_contract, setup_test_env,
...
/// Create a complete test environment with MockVault, Ohloss, and NumberGuess
fn setup_number_guess_test<'a>(
    ...
    // Deploy the number-guess contract with admin and Ohloss address
    let number_guess_addr = env.register(NumberGuessContract, (&admin, &ohloss.address));
    ...
    ohloss.add_game(&number_guess_addr, &developer);
...
fn test_number_guess_game_integration() {
    ...
    let (_admin, _number_guess_addr, number_guess_client, mock_vault, ohloss) =
    ...
    // This will internally call Ohloss to lock FP
    number_guess_client.start_game(&session_id, &player1, &player2, &wager1, &wager2);

    // Verify FP is locked in Ohloss
    let _p1_epoch = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &player1);
    let _p2_epoch = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &player2);
    ...
    // Reveal winner - this also ends the game in Ohloss
    let winner = number_guess_client.reveal_winner(&session_id);

    // Verify FP accounting after game
    let winner_epoch = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &winner);
    let loser = if winner == player1 {
        player2.clone()
    } else {
        player1.clone()
    };
    let _loser_epoch = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &loser);
    ...
    let current_epoch_num = ohloss.get_current_epoch();
    let epoch_info = ohloss.get_epoch(&current_epoch_num);
...
fn test_multiple_number_guess_games() {
    ...
    ohloss.select_faction(&player1, &0); // WholeNoodle
    ohloss.select_faction(&player2, &1); // PointyStick
    ohloss.select_faction(&player3, &0); // WholeNoodle
    ohloss.select_faction(&player4, &2); // SpecialRock
    ...
    // Start both games - this internally calls Ohloss
    ...
    number_guess_client.reveal_winner(&session1); // Ends in Ohloss

    // Game 2
    number_guess_client.make_guess(&session2, &player3, &3);
    number_guess_client.make_guess(&session2, &player4, &8);
    number_guess_client.reveal_winner(&session2); // Ends in Ohloss
...
fn test_cannot_use_unregistered_game() {
    let env = setup_test_env();
    let (_admin, _number_guess_addr, _number_guess_client, mock_vault, ohloss) =
        setup_number_guess_test(&env);
...
fn test_game_can_be_removed_from_registry() {
    let env = setup_test_env();
    let (_admin, number_guess_addr, _number_guess_client, _mock_vault, ohloss) =
        setup_number_guess_test(&env);

    // Verify game is registered
    assert!(ohloss.is_game(&number_guess_addr));

    // Remove game from registry
    ohloss.remove_game(&number_guess_addr);
...
fn test_loser_fp_is_deducted() {
    ...
    let loser_after = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &loser);
...
fn test_winner_fp_returned_loser_fp_spent() {
    ...
    let _p1_during = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &player1);
    let _p2_during = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &player2);
    ...
    // Verify final state
    let winner_final = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &winner);
    let loser = if winner == player1 { player2 } else { player1 };
    let loser_final = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &loser);
...
fn test_asymmetric_wagers() {
    ...
    let _p1_locked = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &player1);
    let _p2_locked = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &player2);
...
fn test_full_epoch_cycle_with_rewards() {
    ...
    // Mint BLND to the Ohloss contract for epoch cycling swaps
    ...
    ohloss.add_game(&number_guess_addr, &game_developer);
    ...
    let epoch0_final = ohloss.get_epoch(&0);
    let current_epoch_num_check = ohloss.get_current_epoch();
    let _epoch1 = ohloss.get_epoch(&current_epoch_num_check);
    ...
    if reward_pool > 0 {
        ...
        for (player, _won_game) in players_and_winners.iter() {
            ...
            let epoch0_data = match ohloss.try_get_epoch_player(&0, &player) {
                Ok(Ok(data)) => data,
            ...
            if player_faction == winning_faction && epoch0_data.total_fp_contributed > 0 {
                ...
                let double_claim_result = ohloss.try_claim_epoch_reward(&player, &0);
                assert!(
    ...
    let epoch1_after_game = ohloss.get_epoch(&1);
