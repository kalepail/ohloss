...
fn setup_complete_game_env<'a>(
    ...
    (game, mock_vault_addr, mock_vault, ohloss, usdc_client)
}
...
fn test_pause_blocks_start_game() {
    ...
    let (game, _vault, mock_vault, ohloss, _usdc) = setup_complete_game_env(&env);
...
fn test_pause_blocks_claim_epoch_reward() {
    let env = setup_test_env();
    let (_game, _vault, _mock_vault, ohloss, _usdc) = setup_complete_game_env(&env);
...
fn test_admin_functions_work_when_paused() {
    ...
    ohloss.add_game(&new_game, &new_developer);
    assert!(ohloss.is_game(&new_game));

    ohloss.remove_game(&new_game);
    assert!(!ohloss.is_game(&new_game));
...
fn test_claim_epoch_reward_before_epoch_finalized() {
    let env = setup_test_env();
    let (_game, _vault, mock_vault, ohloss, _usdc) = setup_complete_game_env(&env);
...
fn test_start_game_with_zero_wager() {
    ...
    let session = 10u32;
    let result = ohloss.try_start_game(&game, &session, &player1, &player2, &0, &100_0000000);
...
fn test_end_game_nonexistent_session() {
    ...
    let result = ohloss.try_end_game(&session, &true);
...
fn test_select_invalid_faction() {
    ...
    let result = ohloss.try_select_faction(&player, &3);
    assert_contract_error(&result, Error::InvalidFaction);

    // Try faction 99
    let result = ohloss.try_select_faction(&player, &99);
    assert_contract_error(&result, Error::InvalidFaction);
}
...
fn test_faction_switch_applies_next_epoch() {
    ...
    let epoch0_player = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &player);
    assert_eq!(
    ...
    ohloss.select_faction(&player, &1);

    // Epoch 0 faction should still be WholeNoodle (locked)
    let epoch0_player_after = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &player);
    assert_eq!(
    ...
    let player_data = ohloss.get_player(&player);
    assert_eq!(
...
fn test_time_multiplier_start_initialized_on_first_game() {
    ...
    let _player_result = ohloss.try_get_player(&player);
...
fn test_get_config() {
    let env = setup_test_env();
    let (_game, vault_addr, _mock_vault, ohloss, _usdc) = setup_complete_game_env(&env);
...
fn test_get_player_nonexistent_user() {
    ...
    let result = ohloss.try_get_player(&nonexistent_user);
...
fn test_get_epoch_player_returns_defaults_before_first_game() {
    ...
    let epoch_data = ohloss.get_epoch_player(&ohloss.get_current_epoch(), &player);
...
fn test_get_epoch_player_errors_without_faction_selection() {
    ...
    let result = ohloss.try_get_epoch_player(&current_epoch, &player);
...
fn test_get_epoch_for_current_and_nonexistent() {
    ...
    let current_epoch_info = ohloss.get_epoch(&current_epoch_num);
    ...
    let result = ohloss.try_get_epoch(&999);
...
fn test_get_faction_standings() {
    ...
    ohloss.end_game(&session, &true);

    // Get faction standings for epoch 0 via get_epoch
    let epoch = ohloss.get_epoch(&0);
